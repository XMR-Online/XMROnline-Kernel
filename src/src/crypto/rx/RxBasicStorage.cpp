...
public:
    ...
    inline void initDataset(uint32_t threads, int priority)
    {
        ...
        if (m_ready) {
            LOG_INFO("%s dataset ready (% ms)", Tags::randomx(), Chrono::steadyMSecs() - ts);
        }
    }
...
private:
    void printAllocStatus(uint64_t ts)
    {
        if (m_dataset->get() != nullptr) {
            ...
            LOG_INFO("%s allocated %zu MB (%zu+%zu) huge pages %s%1.0f%% %u/%u %sJIT (% ms)",
                     Tags::randomx(),
                     pages.size / oneMiB,
			...
			LOG_INFO("%s failed to allocate RandomX dataset, switching to slow mode (% ms)", Tags::randomx(), Chrono::steadyMSecs() - ts);
        }
    }
