    ...
    if (cache_qos) {
        ...
        for (const auto &t : threads) {
            ...
            if (affinity < 0 || std::find(units.begin(), units.end(), affinity) == units.end()) {
				...
				LOG_INFO("%s cache QoS can only be enabled when all mining threads have affinity set", Msr::tag());
                break;
            }
...
bool xmrig::RxMsr::init(const RxConfig &config, const std::vector<CpuThread> &threads)
{
    ...
    if (m_cacheQoS && !Cpu::info()->hasCatL3()) {
		LOG_INFO("%s this CPU doesn't support cat_l3, cache QoS is unavailable", Msr::tag());

        m_cacheQoS = false;
    }

    if ((m_enabled = wrmsr(preset, threads, m_cacheQoS, config.rdmsr()))) {
		LOG_INFO("%s register values for \"%s\" preset have been set successfully (%" PRIu64 " ms)", Msr::tag(), config.msrPresetName(), Chrono::steadyMSecs() - ts);
    }
    else {
		LOG_INFO("%s FAILED TO APPLY MSR MOD, HASHRATE WILL BE LOW", Msr::tag());
    }
...
void xmrig::RxMsr::destroy()
{
    ...
    if (!wrmsr(items, std::vector<CpuThread>(), m_cacheQoS, false)) {
		LOG_INFO("%s failed to restore initial state (% ms)", Msr::tag(), Chrono::steadyMSecs() - ts);
    }
}
