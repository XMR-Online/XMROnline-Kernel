...
{
	LOG_INFO("%s#%u skipped (%s)", Tags::randomx(), nodeId, reason);
}


static inline void printDatasetReady(uint32_t nodeId, uint64_t ts)
{
    LOG_INFO("%s#%u dataset ready (% ms)", Tags::randomx(), nodeId, Chrono::steadyMSecs() - ts);
}
...
public:
    ...
    inline bool createDatasets(bool hugePages, bool oneGbPages)
    {
        ...
        if (m_datasets.empty()) {
			...
			LOG_INFO("%sfailed to allocate RandomX datasets, switching to slow mode (% ms)", Tags::randomx(), Chrono::steadyMSecs() - ts);
        }
        else {
            if (m_cache) {
...
private:
    ...
    static void allocateCache(RxNUMAStoragePrivate *d_ptr, uint32_t nodeId, bool hugePages)
    {
        ...
        if (!cache->get()) {
            ...
            LOG_INFO("%s failed to allocate RandomX memory (% ms)", Tags::randomx(), Chrono::steadyMSecs() - ts);
    ...
    static void printAllocStatus(RxDataset *dataset, uint32_t nodeId, uint64_t ts)
    {
        ...
        LOG_INFO("%s #%u allocated %zu MB huge pages %s%3.0f%% (% ms)",
                 Tags::randomx(),
                 nodeId,
                 pages.size / oneMiB,
    ...
    static void printAllocStatus(RxCache *cache, uint32_t nodeId, uint64_t ts)
    {
        ...
        LOG_INFO("%s #%u allocated %4zu MB huge pages %s%3.0f%% %sJIT (% ms)",
                 Tags::randomx(),
                 nodeId,
                 cache->size() / oneMiB,
                 (pages.isFullyAllocated() ? GREEN_BOLD_S : RED_BOLD_S),
    ...
    void printAllocStatus(uint64_t ts) const
    {
        ...
        LOG_INFO("%s allocated %4zu MB huge pages %s%3.0f%% %u/%u (% ms)",
                 Tags::randomx(),
                 pages.size / oneMiB,
