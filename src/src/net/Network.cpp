...
#   endif
    ...
    if (fingerprint != nullptr) {
        LOG_INFO("fingerprint(SHA-256): \"%s\"", fingerprint);
    }
}
...
void xmrig::Network::onPause(IStrategy *strategy)
{
    ...
    if (!m_strategy->isActive()) {
		LOG_INFO("%s no active pools, stop mining", Tags::network());
...
void xmrig::Network::onResultAccepted(IStrategy *, IClient *, const SubmitResult &result, const char *error)
{
	...
	if (error) {
		LOG_INFO("%s rejected (%" PRId64 "/%" PRId64 ") diff %" PRIu64 "%s \"%s\" (%" PRIu64 " ms)",
			backend_tag(result.backend), m_state->accepted(), m_state->rejected(), diff, scale, error, result.elapsed);
	}
	else {
		LOG_INFO("%s accepted (%" PRId64 "/%" PRId64 ") diff %" PRIu64 "%s (%" PRIu64 " ms)",
...
#   endif
    {
		...
		LOG_INFO("%s new job from pool diff %" PRIu64 "%s height %" PRIu64 "%s",
			Tags::network(), diff, scale, job.height(), tx_buf);
    }
