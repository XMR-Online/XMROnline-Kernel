...
public:
    ...
    {
        if (m_started == 0) {
			LOG_INFO("%s disabled (failed to start threads)", Tags::cpu());

            return;
        }

        LOG_INFO("%s READY threads %s%zu/%zu (%zu)" CLEAR " huge pages %s%1.0f%% %zu/%zu" CLEAR " memory " CYAN_BOLD("%zu KB") BLACK_BOLD(" (%" PRIu64 " ms)"),
                 Tags::cpu(),
...
void xmrig::CpuBackend::setJob(const Job &job)
{
    ...
    if (d_ptr->profileName.isNull() || threads.empty()) {
		LOG_INFO("%s disabled (no suitable configuration found)", Tags::cpu());
...
void xmrig::CpuBackend::stop()
{
    ...
    LOG_INFO("%s stopped (%" PRIu64 " ms)", Tags::cpu(), Chrono::steadyMSecs() - ts);
}
