...
#       endif
        ...
        for (const OclDevice &device : devices) {
            Log::print( CYAN_BOLD("#%zu") YELLOW(" %s") " %s " WHITE_BOLD("%u MHz") " cu:" WHITE_BOLD("%u") " mem:" CYAN("%zu/%zu") " MB", "CPU",
                       device.index(),
...
void xmrig::OclBackend::setJob(const Job &job)
{
    ...
    if (d_ptr->profileName.isNull() || threads.empty()) {
		LOG_INFO("%s disabled (no suitable configuration found)", Tags::opencl());

        return stop();
    }

    if (!d_ptr->context.init(d_ptr->devices, threads)) {
		LOG_INFO("%s disabled (OpenCL context unavailable)", Tags::opencl());
