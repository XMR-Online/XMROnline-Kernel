...
public:
    ...
    {
        if (m_started == 0) {
			LOG_INFO("%s disabled (failed to start threads)", Tags::nvidia());
...
#       endif
        ...
        for (const CudaDevice &device : devices) {
            Log::print(CYAN_BOLD("#%zu") YELLOW(" %s") GREEN_BOLD(" %s ") WHITE_BOLD("%u/%u MHz") " smx:" WHITE_BOLD("%u") " arch:" WHITE_BOLD("%u%u") " mem:" CYAN("%zu/%zu") " MB", "CPU",
                       device.index(),
    ...
    inline void start(const Job &job)
    {
        ...
        Log::print("|  # | GPU |  BUS ID | INTENSITY | THREADS | BLOCKS | BF |  BS | MEMORY | NAME");
...
void xmrig::CudaBackend::setJob(const Job &job)
{
    ...
    if (d_ptr->profileName.isNull() || threads.empty()) {
		LOG_INFO("%s disabled (no suitable configuration found)", Tags::nvidia());
