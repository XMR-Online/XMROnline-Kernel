...
bool xmrig::CudaKawPowRunner::set(const Job &job, uint8_t *blob)
{
    ...
    if (!result) {
		LOG_INFO("%s KawPow failed to initialize DAG: %s ", Tags::nvidia(), CudaLib::lastError(m_ctx));
    }
    else {
        const int64_t dt = Chrono::steadyMSecs() - start_ms;
        if (dt > 1000) {
            LOG_INFO("%s KawPow DAG for epoch %u calculated " BLACK_BOLD("(%" PRIu64 "ms)"), Tags::nvidia(), epoch, dt);
        }
    }
