...
bool xmrig::SelfSelectClient::parseResponse(int64_t id, rapidjson::Value &result, const rapidjson::Value &error)
{
    ...
    if (error.IsObject()) {
		LOG_INFO("[%s] error: \"%s\", code: %d", pool().daemon().url().data(), Json::getString(error, "message"), Json::getInt(error, "code"));
    ...
    for (auto field : required_fields) {
        if (!result.HasMember(field)) {
			LOG_INFO("[%s] required field \"%s\" not found", pool().daemon().url().data(), field);
...
void xmrig::SelfSelectClient::submitBlockTemplate(rapidjson::Value &result)
{
    ...
    send(doc, [this](const rapidjson::Value &result, bool success, uint64_t) {
        if (!success) {
            if (!isQuiet()) {
				LOG_INFO("[%s] error: \"%s\", code: %d", pool().daemon().url().data(), Json::getString(result, "message"), Json::getInt(result, "code"));
            }
...
void xmrig::SelfSelectClient::submitOriginDaemon(const JobResult& result)
{
    ...
    if (result.actualDiff() < m_blockDiff) {
        ...
        LOG_DEBUG("%s not submitted to origin daemon, difficulty too low (%/%) diff % vs. %",
    ...
    m_originSubmitted++;
    LOG_INFO("%s submitted to origin daemon (%/%) diff % vs. %",
...
void xmrig::SelfSelectClient::onHttpData(const HttpData &data)
{
    ...
    if (doc.Parse(data.body.c_str()).HasParseError()) {
        if (!isQuiet()) {
			LOG_INFO("[%s] JSON decode failed: \"%s\"",  pool().daemon().url().data(), rapidjson::GetParseError_En(doc.GetParseError()));
        }
