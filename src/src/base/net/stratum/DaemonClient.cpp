...
void xmrig::DaemonClient::connect()
{
    auto connectError = [this](const char *message) {
        if (!isQuiet()) {
			LOG_INFO("%s connect error: \"%s\"", tag(), message);
        }
...
bool xmrig::DaemonClient::parseJob(const rapidjson::Value &params, int *code)
{
    auto jobError = [this, code](const char *message) {
        if (!isQuiet()) {
			LOG_INFO("%s job error: \"%s\"", tag(), message);
        }
...
bool xmrig::DaemonClient::parseResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
    ...
    if (error.IsObject()) {
        ...
        if (!handleSubmitResponse(id, message) && !isQuiet()) {
			LOG_INFO("[%s:%d] error: \"%s\", code: %d", m_pool.host().data(), m_pool.port(), message, error["code"].GetInt());
        }
...
void xmrig::DaemonClient::onZMQConnect(uv_connect_t* req, int status)
{
    ...
    if (status < 0) {
		LOG_INFO("%s ZMQ connect error: \"%s\"", client->tag(), uv_strerror(status));
        client->retry();
        return;
    }
...
bool xmrig::DaemonClient::ZMQWrite(const char* data, size_t size)
{
	...
	LOG_INFO("%s ZMQ write failed, rc = %d ", tag(), rc);
    ZMQClose();
    return false;
}
...
{
    if (nread <= 0) {
		LOG_INFO("%s ZMQ read failed, nread = % ", tag(), nread);
        ZMQClose();
        return;
    }
    ...
    do {
        ...
        case ZMQ_GREETING_1:
            if (m_ZMQRecvBuf.size() >= kZMQGreetingSize1) {
				...
				LOG_INFO("%s ZMQ handshake failed: invalid greeting format", tag());
                ZMQClose();
            }
            return;
        ...
        case ZMQ_GREETING_2:
            if (m_ZMQRecvBuf.size() >= sizeof(kZMQGreeting)) {
				...
				LOG_INFO("%s ZMQ handshake failed: invalid greeting format 2", tag());
                ZMQClose();
        ...
        case ZMQ_HANDSHAKE:
            if (m_ZMQRecvBuf.size() >= 2) {
                if (m_ZMQRecvBuf[0] != 4) {
					LOG_INFO("%s ZMQ handshake failed: invalid handshake format", tag());
                    ZMQClose();
                    return;
                }
                ...
                if (size < 18) {
					LOG_INFO("%s ZMQ handshake failed: invalid handshake size", tag());
                    ZMQClose();
                    return;
                }
                ...
                if (memcmp(m_ZMQRecvBuf.data() + 2, kZMQHandshake + 2, 18) != 0) {
					LOG_INFO("%s ZMQ handshake failed: invalid handshake data", tag());
                    ZMQClose();
                    return;
                }
...
#   endif
    ...
    do {
        ...
        {
			LOG_INFO("%s ZMQ message is too large, size = %" PRIu64 " bytes", tag(), size);
            ZMQClose();
            return;
        }
