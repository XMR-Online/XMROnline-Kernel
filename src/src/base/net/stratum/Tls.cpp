...
{
    if (cert == nullptr) {
		LOG_INFO("[%s] Failed to get server certificate", m_client->url());

        return false;
    }

    if (!verifyFingerprint(cert)) {
		LOG_INFO("[%s] Failed to verify server certificate fingerprint", m_client->url());

        const char *fingerprint = m_client->m_pool.fingerprint();
        if (strlen(m_fingerprint) == 64 && fingerprint != nullptr) {
			LOG_INFO("\"%s\" was given", m_fingerprint);
			LOG_INFO("\"%s\" was configured", fingerprint);
        }
