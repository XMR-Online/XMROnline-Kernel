...
int64_t xmrig::Client::send(const rapidjson::Value &obj)
{
    ...
    if (size > kMaxSendBufferSize) {
		LOG_INFO("%s send failed: \"max send buffer size exceeded: %zu\"", tag(), size);
        close();
...
void xmrig::Client::onResolved(const DnsRecords &records, int status, const char *error)
{
    ...
    if (status < 0 && records.isEmpty()) {
        if (!isQuiet()) {
			LOG_INFO("%s DNS error: \"%s\"", tag(), error);
        }
...
#   endif
    ...
    if (!isQuiet()) {
		LOG_INFO("[net]- duplicate job received, reconnect");
    }
...
bool xmrig::Client::verifyAlgorithm(const Algorithm &algorithm, const char *algo) const
{
    if (!algorithm.isValid()) {
        if (!isQuiet()) {
            if (algo == nullptr) {
				LOG_INFO("%s unknown algorithm, make sure you set \"algo\" or \"coin\" option", tag(), algo);
            }
            else {
				LOG_INFO("%s unsupported algorithm \"%s\" detected, reconnect", tag(), algo);
            }
        }
    ...
    if (!ok && !isQuiet()) {
		LOG_INFO("%s incompatible/disabled algorithm \"%s\" detected, reconnect", tag(), algorithm.name());
    }
...
bool xmrig::Client::write(const uv_buf_t &buf)
{
    ...
    if (!isQuiet()) {
		LOG_INFO("%s write error: \"%s\"", tag(), uv_strerror(rc));
    }
...
void xmrig::Client::parse(char *line, size_t len)
{
    ...
    if (len < 22 || line[0] != '{') {
        if (!isQuiet()) {
			LOG_INFO("%s JSON decode failed", tag());
        }
    ...
    if (doc.ParseInsitu(line).HasParseError()) {
        if (!isQuiet()) {
			LOG_INFO("%s JSON decode failed: \"%s\"", tag(), rapidjson::GetParseError_En(doc.GetParseError()));
        }
    ...
    if (method && strcmp(method, "client.reconnect") == 0) {
        ...
        if (!params.IsArray()) {
			LOG_INFO("%s invalid client.reconnect notification: params is not an array", tag());
            return;
        }
        ...
        if (arr.Empty()) {
			LOG_INFO("%s invalid client.reconnect notification: params array is empty", tag());
            return;
        }

        if (arr.Size() != 2) {
			LOG_INFO("%s invalid client.reconnect notification: params array has wrong size", tag());
            return;
        }

        if (!arr[0].IsString()) {
			LOG_INFO("%s invalid client.reconnect notification: host is not a string", tag());
            return;
        }

        if (!arr[1].IsString()) {
			LOG_INFO("%s invalid client.reconnect notification: port is not a string", tag());
            return;
        }

        std::stringstream s;
        s << arr[0].GetString() << ":" << arr[1].GetString();
		LOG_INFO("%s client.reconnect to %s", tag(), s.str().c_str());
...
void xmrig::Client::parseResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
    ...
    if (error.IsObject()) {
        ...
        if (!handleSubmitResponse(id, message) && !isQuiet()) {
			LOG_INFO("%s error: \"%s\", code:%d ", tag(), message, Json::getInt(error, "code"));
        }
    ...
    if (id == 1) {
        ...
        if (!parseLogin(result, &code)) {
            if (!isQuiet()) {
				LOG_INFO("%s login error code: %d ", tag(), code);
            }
...
void xmrig::Client::read(ssize_t nread, const uv_buf_t *buf)
{
    ...
    if (nread < 0) {
        if (!isQuiet()) {
			LOG_INFO("%s read error: \"%s\"", tag(), uv_strerror(static_cast<int>(nread)));
        }
...
void xmrig::Client::onConnect(uv_connect_t *req, int status)
{
    ...
    if (status < 0) {
        if (!client->isQuiet()) {
			LOG_INFO("%s connect error: \"%s\"", client->tag(), uv_strerror(status));
        }
