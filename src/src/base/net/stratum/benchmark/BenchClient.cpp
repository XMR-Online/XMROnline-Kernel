...
#   endif
	...
	LOG_INFO("%s benchmark finished in %.3f seconds (%.1f h/s) hash sum = %s%016" PRIX64 CLEAR, tag(), dt, BenchState::size() / dt, color, result);
...
void xmrig::BenchClient::start()
{
	...
	LOG_INFO("%s start benchmark hashes %u%s algo %s",
               tag(),
...
{
	LOG_INFO("%s %s: \"%s\"", tag(), label ? label : "benchmark failed", message);
    printExit();
