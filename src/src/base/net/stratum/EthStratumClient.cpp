...
#   endif
    ...
    if (result.diff == 0) {
		LOG_INFO("%s result.diff is 0", tag());
        close();
...
void xmrig::EthStratumClient::parseNotification(const char *method, const rapidjson::Value &params, const rapidjson::Value &)
{
    ...
    if (strcmp(method, "mining.set_extranonce") == 0) {
        if (!params.IsArray()) {
			LOG_INFO("%s invalid mining.set_extranonce notification: params is not an array", tag());
            return;
        }
        ...
        if (arr.Empty()) {
			LOG_INFO("%s invalid mining.set_extranonce notification: params array is empty", tag());
            return;
        }
...
#   ifdef XMRIG_ALGO_GHOSTRIDER
    if (strcmp(method, "mining.set_difficulty") == 0) {
        if (!params.IsArray()) {
			LOG_INFO("%s invalid mining.set_difficulty notification: params is not an array", tag());
            return;
        }
        ...
        if (arr.Empty()) {
			LOG_INFO("%s invalid mining.set_difficulty notification: params array is empty", tag());
            return;
        }

        if (!arr[0].IsDouble() && !arr[0].IsUint64()) {
			LOG_INFO("%s invalid mining.set_difficulty notification: difficulty is not a number", tag());
            return;
        }
...
#   endif
    ...
    if (strcmp(method, "mining.notify") == 0) {
        if (!params.IsArray()) {
			LOG_INFO("%s invalid mining.notify notification: params is not an array", tag());
            return;
        }
        ...
        if (arr.Size() < min_arr_size) {
			LOG_INFO("%s invalid mining.notify notification: params array has wrong size", tag());
            return;
        }

        if (!arr[0].IsString()) {
			LOG_INFO("%s invalid mining.notify notification: invalid job id", tag());
            return;
        }
...
#       ifdef XMRIG_ALGO_GHOSTRIDER
        if (algo.id() == Algorithm::GHOSTRIDER_RTM) {
            ...
            if (!arr[1].IsString() || !arr[2].IsString() || !arr[3].IsString() || !arr[4].IsArray() || !arr[5].IsString() || !arr[6].IsString() || !arr[7].IsString()) {
				LOG_INFO("%s invalid mining.notify notification: invalid param array", tag());
                return;
            }
            ...
            for (int i = 0, n = merkle_branches.Size(); i < n; ++i) {
                ...
                if (buf.size() != 32) {
					LOG_INFO("%s invalid mining.notify notification: param 4 is invalid", tag());
                    return;
                }
            ...
            if (blob.size() != 76 * 2) {
				LOG_INFO("%s invalid mining.notify notification: invalid blob size", tag());
                return;
            }
...
#       endif
        ...
        if (!ok) {
            if (!isQuiet()) {
				LOG_INFO("[%s] incompatible/disabled algorithm \"%s\" detected, reconnect", url(), algo.name());
            }
            close();
            return;
        }
