...
bool xmrig::TlsContext::load(const TlsConfig &config)
{
    ...
    if (m_ctx == nullptr) {
		LOG_INFO("Unable to create SSL context");
    ...
    if (SSL_CTX_use_certificate_chain_file(m_ctx, cert) <= 0) {
		LOG_INFO("SSL_CTX_use_certificate_chain_file(\"%s\") failed.", config.cert());
    ...
    if (SSL_CTX_use_PrivateKey_file(m_ctx, key, SSL_FILETYPE_PEM) <= 0) {
		LOG_INFO("SSL_CTX_use_PrivateKey_file(\"%s\") failed.", config.key());
...
bool xmrig::TlsContext::setCiphers(const char *ciphers)
{
	...
	LOG_INFO("SSL_CTX_set_cipher_list(\"%s\") failed.", ciphers);
...
#   endif
	...
	LOG_INFO("SSL_CTX_set_ciphersuites(\"%s\") failed.", ciphersuites);
...
bool xmrig::TlsContext::setDH(const char *dhparam)
{
    ...
    if (dhparam != nullptr) {
        ...
        if (bio == nullptr) {
			LOG_INFO("BIO_new_file(\"%s\") failed.", dhparam);
        ...
        if (dh == nullptr) {
			LOG_INFO("PEM_read_bio_DHparams(\"%s\") failed.", dhparam);
    ...
    if (rc == 0) {
		LOG_INFO("SSL_CTX_set_tmp_dh(\"%s\") failed.", dhparam);
