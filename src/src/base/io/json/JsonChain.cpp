...
bool xmrig::JsonChain::addFile(const char *fileName)
{
    ...
    if (doc.HasParseError()) {
        ...
        if (Json::convertOffset(fileName, offset, line, pos, s)) {
			...
			LOG_INFO("%s<line:%zu, position:%zu>: \"%s\"", fileName, line, pos, GetParseError_En(doc.GetParseError()));
        }
        else {
			LOG_INFO("%s<offset:%zu>: \"%s\"", fileName, offset, GetParseError_En(doc.GetParseError()));
        }
    }
    else {
