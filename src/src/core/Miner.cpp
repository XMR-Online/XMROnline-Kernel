...
#       endif
		...
		LOG_INFO("%s speed (%s/%s)to(%s/%s)", Tags::miner(), Hashrate::format(speed[0] * scale, num, 16), h, Hashrate::format(maxHashrate[algorithm] * scale, num + 16 * 3, 16), h);
...
void xmrig::Miner::setEnabled(bool enabled)
{
    ...
    if (d_ptr->controller->config()->isPauseOnBattery() && d_ptr->battery_power && enabled) {
        LOG_INFO("%s can't resume while on battery power", Tags::miner());
    ...
    if (enabled) {
        LOG_INFO("%s resumed", Tags::miner());
    }
    else {
        if (d_ptr->battery_power) {
            LOG_INFO("%s paused", Tags::miner());
        }
    }
