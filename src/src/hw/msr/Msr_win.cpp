    ...
    bool uninstall()
    {
        ...
        if (!reuse) {
            ...
            if (!DeleteService(service)) {
				LOG_INFO("%s failed to remove WinRing0 driver, error %u", Msr::tag(), GetLastError());
                result = false;
            }
        }
...
xmrig::Msr::Msr() : d_ptr(new MsrPrivate())
{
    ...
    if (!d_ptr->manager) {
        if ((err = GetLastError()) == ERROR_ACCESS_DENIED) {
			LOG_INFO("%s to access MSR registers Administrator privileges required.", tag());
        }
        else {
			LOG_INFO("%s failed to open service control manager, error %u", tag(), err);
        }
    ...
    if (err != ERROR_SUCCESS) {
		LOG_INFO("%s failed to get path to driver, error %u", tag(), err);
        return;
    }
    ...
    if (d_ptr->service) {
		LOG_INFO("%s service WinRing0_1_2_0 already exists", tag());
    ...
    if (d_ptr->driver != INVALID_HANDLE_VALUE) {
		LOG_INFO("%s service WinRing0_1_2_0 already exists, but with a different service name", tag());
        d_ptr->reuse = true;
        return;
    }
    ...
    if (!d_ptr->reuse) {
        ...
        if (!d_ptr->service) {
			LOG_INFO("%s failed to install WinRing0 driver, error %u", tag(), GetLastError());
        ...
        if (!StartService(d_ptr->service, 0, nullptr)) {
            ...
            if (err != ERROR_SERVICE_ALREADY_RUNNING) {
                if (err == ERROR_FILE_NOT_FOUND) {
					LOG_INFO("%s failed to start WinRing0 driver: \"WinRing0x64.sys not found\"", tag());
                }
                else {
					LOG_INFO("%s failed to start WinRing0 driver, error %u", tag(), err);
                }
    ...
    d_ptr->driver = CreateFileW(L"\\\\.\\" SERVICE_NAME, GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
    if (d_ptr->driver == INVALID_HANDLE_VALUE) {
		LOG_INFO("%s failed to connect to WinRing0 driver, error %u", tag(), GetLastError());;
    }
}
